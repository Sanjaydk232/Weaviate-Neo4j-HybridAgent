...
def virtualGraphSearch(userQ):
    cypher = """
        ...
        RETURN p.product_name                                              AS product,
               ...
               apoc.create.vRelationship(c, 'VIRTUALLY_ORDERED',
                   {order_count: order_count, avg_review: avg_review}, p) AS vRel
    ...
    try:
        ...
        if not rows:
            return ""
        lines = ["Virtual Graph Projection (Customer -> Product, Order node collapsed):"]
        for row in rows:
            lines.append(
...
async def HybridSearch(userQ, session_id=None):
    ...
    async with MemoryClient(memorySettings) as memory:
        ...
        if t2cR:
            if usingNeo4j:
                dataB = "Virtual Graph Data (APOC Projection)" if vgUsed else "Graph Data (Neo4j)"
            else:
                dataB = "Semantic Data (Weaviate)"
            Description = (
